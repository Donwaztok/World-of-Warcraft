...
if (isset($_SESSION['user']) != "") {
	...
	if (isset($_POST['btn-login'])) {
		...
		if (empty($name)) {
			$error = true;
			$errMSG = "Por favor digite um login.";
		} else if (strlen($name) < 3) {
			$error = true;
			$errMSG = "login precisa ter no minimo 3 caracteres.";
		}
		//Validar Senha
		if (empty($pass)) {
			$error = true;
			$errMSG = "Por favor digite uma senha.";
		} else if (strlen($pass) < 4) {
			$error = true;
			$errMSG = "Senha precisa ter mais de 4 caracteres.";
		}
		...
		if (!$error) {
			...
			if ($res) {
				$error = false;
				$errMSG = "Registrado com sucesso, você pode se conectar agora.";
				unset($name);
				unset($pass);
			} else {
				$error = true;
                     ...
                     <div class=login-wrap>
                      <div class=login-html>
						...
						if (file_exists("Images/users/" . $userRow["id"] . "." . $userRow["portrait_type"])) {
							...
							echo "<img src=Images/users/0.png>";
						}
